...
INCLUDE_STRING=`git diff --name-only --cached --diff-filter=d "${PATTERN}" | sed ':a;N;$!ba;s/\n/;/g'`
...
if [[ "$OSTYPE" == "msys"* ]]; then
    ...
    ./ci/resharper/cleanupcode.exe --profile="Built-in: Reformat Code" "$SLN" --include="$INCLUDE_STRING"
elif [[ "$OSTYPE" == "cygwin" ]]; then
    #Cygwin terminal emulator
    ./ci/resharper/cleanupcode.exe --profile="Built-in: Reformat Code" "$SLN" --include="$INCLUDE_STRING"
else
    sh ./ci/resharper/cleanupcode.sh --profile="Built-in: Reformat Code" "$SLN" --include="$INCLUDE_STRING"
fi
